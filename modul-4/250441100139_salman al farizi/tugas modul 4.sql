...
WHERE 
d.bidang_keahlian IN ('Pemrograman', 'Basis Data');
...
krs k ON m.id_mahasiswa = k.id_mahasiswa
JOIN 
mata_kuliah mk ON k.id_mk = mk.id_mk
WHERE 
k.semester_ambil BETWEEN 3 AND 5
ORDER BY 
k.semester_ambil ASC;
...
mata_kuliah mk ON k.id_mk = mk.id_mk
WHERE 
m.program_studi = 'Sistem Informasi' 
AND mk.sks > 2
ORDER BY 
mk.sks DESC, 
...
mahasiswa AS m
JOIN 
krs AS k ON m.id_mahasiswa = k.id_mahasiswa
JOIN 
mata_kuliah AS mk ON k.id_mk = mk.id_mk
JOIN 
dosen AS d ON mk.id_dosen = d.id_dosen
WHERE 
m.angkatan BETWEEN 2023 AND 2025
